...
class NWChem(logfileparser.Logfile):
    ...
    def extract(self, inputfile, line):
        ...
        # ...
#vama this is temporal
#vama this is temporal
#vama this is temporal
#vama this is temporal
        if line.split() == ['convergence', 'iter', 'energy', 'DeltaE', 'RMS-Dens', 'Diis-err', 'time'] and False:
        ...
        if "Task  times  cpu" in line:
            if not hasattr(self, 'walltime'):
                time = line.split()[3]
                self.walltime = float(time.split('s')[0])
        ...
        if "Final" in line and "Molecular Orbital Analysis" in line:
            ...
            if not hasattr(self, 'nooccnos') or (len(self.nooccnos) > alphabeta):
                self.nooccnos = []
            self.nooccnos.append(mooccnos)
